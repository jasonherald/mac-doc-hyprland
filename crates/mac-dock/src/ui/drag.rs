...
pub fn setup_drag_source(
    ...
    // Drag begin: snapshot the icon for the cursor, then hide the original
    ...
    drag_source.connect_drag_begin(move |source, _drag| {
        ...
        if let Some(widget) = source.widget() {
            ...
            // Hide the entire dock-item container (picked it up)
    ...
    drag_source.connect_drag_end(move |source, _drag, delete_data| {
        let drag_idx = state_end.borrow().drag_source_index;
        state_end.borrow_mut().drag_source_index = None;

        // Show the item again (in case drop was on dock and rebuild handles it)
        if let Some(widget) = source.widget()
            && let Some(parent) = widget.parent() {
                parent.set_visible(true);
            }

        // If the drop was NOT accepted by our dock drop target, unpin the item
        if !delete_data
...
pub fn setup_dock_drop_target(
    ...
    // Motion: calculate drop index and show icon preview
    ...
    drop_target.connect_motion(move |_target, x, _y| {
        ...
        if *current != Some(new_idx) {
            ...
            let preview = create_icon_preview(&s, size);
            drop(s);
    ...
    // Remove placeholder when drag leaves
